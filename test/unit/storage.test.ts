...
describe("storage", () => {
  it("creates schema on fresh :memory: db", () => {
    const s = new PeffleStorage(":memory:");
    const row = s.db
  ...
  it("persists data across reopen", () => {
    const dir = mkdtempSync(join(tmpdir(), "peffle-store-"));
    const path = join(dir, "ledger.db");
    const a = new PeffleStorage(path);
    a.upsertKill("agent-x", "test");
    a.close();
    const b = new PeffleStorage(path);
  ...
  it("creates db file with 0600 on unix", () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), "peffle-perm-"));
    const path = join(dir, "ledger.db");
    const s = new PeffleStorage(path);
    s.close();
